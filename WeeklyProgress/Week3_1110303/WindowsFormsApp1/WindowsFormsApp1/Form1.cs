...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        Thread Th; // 宣告監聽用執行緒
        String MyName; // 我的名稱
        ArrayList ips = new ArrayList(); // 線上客戶IP 列表
        ...
        private string MyIP()
        {
            ...
            foreach (IPAddress it in ip) // 列舉各個IP
            {
                ...
                // 如果是IPv4 格式
                {
                    ips=it.ToString(); // 傳回此IP 字串
                    //return it.ToString;
                }
            }
            return ips; // 找不到合格IP，回傳空字串
        }
        // 上線或離線的選擇
        ...
        private void textBox2_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter) // 按下Enter
            {
                ...
                else// 發送私密訊息
                {
                    ...
                    listBox2.Items.Add(MyName + " to " + listBox1.SelectedItem.
                    ToString() + "：" + textBox2.Text); // 訊息寫入看板
                }
        ...
        private void Send(string ToIP, string msg, string toWhom)
        {
            ...
            UdpClient V = new UdpClient(ToIP, Port); // 建立UDP 通訊物件
            V.Send(B, B.Length); // 發送資料
        }
        // 監聽副程式
        private void Listen()
        {
            U = new UdpClient(Port); // 建立UDP 通訊物件
                                     // 接聽端點(IP+Port)
