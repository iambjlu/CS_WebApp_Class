...
using Microsoft.VisualBasic.PowerPacks;// 匯入VB 向量繪圖功能
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            this.Controls.Add(C); // 加入畫布C 到表單
            D = new ShapeContainer(); // 建立畫布( 遠端繪圖用)
            this.Controls.Add(D); // 加入畫布D 到表單
        }
        // 啟動監聽按鍵程序
        ...
        private void Listen()
        {
            ...
            U = new UdpClient(Port); // 建立UDP 監聽器
                                     // 建立本機端點資訊
            IPEndPoint EP = new IPEndPoint(IPAddress.Parse("127.0.0.1"), Port);
            while (true) // 持續監聽的無限迴圈→有訊息(True) 就處理，無訊息就等待！
            {
                byte[] B = U.Receive(ref EP); // 訊息到達時讀取資訊到B 陣列
                string A = Encoding.Default.GetString(B); // 翻譯B 陣列為字串A
                string[] Z = A.Split('_'); // 切割顏色與座標資訊
                string[] Q = Z[1].Split('/'); // 切割座標點資訊
                Point[] R = new Point[Q.Length]; // 宣告座標點陣列
                for (int i = 0; i < Q.Length; i++)
                {
                    string[] K = Q[i].Split(','); // 切割X 與Y 座標
                    R[i].X = int.Parse(K[0]); // 定義第i 點X 座標
                    R[i].Y = int.Parse(K[1]); // 定義第i 點Y 座標
                }
        ...
        private void Form1_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            UdpClient S = new UdpClient(textBox1.Text, Port); // 建立UDP 物件
            ...
            byte[] B = Encoding.Default.GetBytes(p); // 翻譯p 字串為B 陣列
